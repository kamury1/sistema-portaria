...
from banco import CAMINHO_BANCO


def pasta_backups():
    """Retorna a pasta onde os backups locais serão armazenados."""

    pasta_base = os.path.dirname(CAMINHO_BANCO)
...
def fazer_backup_banco(janela=None):
    """
    Cria uma cópia consistente do banco SQLite usando
    a API de backup do próprio SQLite.

    O arquivo é salvo na pasta 'backups'
    ...
    nome_arquivo = agora.strftime(
        "portaria_backup_%Y-%m-%d_%H-%M-%S.db"
    )
    ...
    try:
        ...
        destino_conexao = sqlite3.connect(
            destino
        )

        origem_conexao.backup(
            destino_conexao
        )
